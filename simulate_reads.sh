...
run_simulations(){
  if [ $# -ne 2 ]
    then
      echo "Incorrect number of arguments supplied. Two arguments should be passed to this function, the path to the directory in which the data is stored and the number of reads to simulate."
      exit 1
  fi
  memory=`pwd`
  cd $1
  ...
  do
    ...
    bsub -n8 -R"span[hosts=1]" -c 99999 -G team_hemberg -q normal -o $TEAM/temp.logs/output.$filename -e $TEAM/temp.logs/error.$filename -R"select[mem>100000] rusage[mem=100000]" -M100000 simulate $i $1 $3
...
#Function which performs RSEM simulations. Takes 3 args, the filename of the cell, the directory in which it is stored and the number of reads to simulate.
simulate() {
